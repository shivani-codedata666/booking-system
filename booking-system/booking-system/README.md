# Slotwise — Booking & Reservation System
...
1. A **database transaction** wrapping the overlap-check + insert as one atomic unit, so no other write can interleave between the check and the write.
2. An **interval-overlap query** (`start_a < end_b AND start_b < end_a`) rather than a naive exact-match check, so partially-overlapping bookings are caught too.
3. A **UNIQUE index** on `(resource_id, start_time)` for confirmed bookings as a second line of defense.
